  ...
  return (
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        >
          <Command className="shadow-md bg-black rounded-none p-0.5 py-1" shouldFilter={!error}>
            ...
            <CommandList className="px-2">
              {error ? (
                <div className="p-4 text-sm text-destructive font-medium">{error}</div>
              ) : (
                <>
                  <CommandEmpty className="relative flex cursor-default items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-hidden text-white select-none data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 data-[selected=true]:bg-neutral-800 data-[selected=true]:text-white [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 [&_svg:not([class*='text-'])]:text-muted-foreground">
                    No results found for "{validatedQuery}"
                  </CommandEmpty>
                  <CommandGroup heading="Suggestions" className="text-white text-center">
