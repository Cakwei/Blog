...
export const Route = createFileRoute("/_protected/posts/")({
    loader: async ({ context }) => {
        ...
        await context.queryClient.ensureQueryData(postsQueryOptions(userId));
    },
...
function AdminPostsPage() {
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16 space-y-10 text-(--text) bg-(--bg) selection:bg-(--link)/20 selection:text-(--link)">
            ...
            {/* Content Listing */}
            <Suspense fallback={<PostListSkeletonContainer count={5} />}>
                <PostListContent />
            </Suspense>
        </div>
    );
...
function PostListSkeleton() {
    return (
        <div className="w-[75.5dvh] flex flex-col sm:flex-row sm:items-center gap-4 px-5 py-4 animate-pulse">
            {/* Thumbnail Skeleton matching PostRow dimensions */}
            <Skeleton className="w-full sm:w-16 h-28 sm:h-16 rounded-2xl bg-(--bg) flex-shrink-0" />

            {/* Details & Tags Skeleton */}
            ...
            {/* Actions Skeleton matching row button positioning */}
            <div className="flex items-center gap-2 self-end sm:self-center flex-shrink-0 pt-2 sm:pt-0">
