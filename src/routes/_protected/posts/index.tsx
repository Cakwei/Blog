...
const getOwnPosts = createServerFn().handler(async () => {
	...
	const posts = await prisma.post.findMany({
		...
		select: {
			...
			isFeatured: true,
			slug: true,
			published: true, // Fixed: maps to database boolean column
			title: true,
			user: true,
			views: true,
			categories: {
				// Fixed: matches schema relation name
				select: {
					id: true,
					slug: true,
					name: true,
...
function PostRow({ post }: { post: any }) {
	// Fixed: evaluate draft status using the boolean `published` property
