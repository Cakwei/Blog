...
import { ArrowLeft, Calendar, ChevronDown, Compass, ImageIcon, Search, Sparkles, User } from "lucide-react";
import {
    createContext,
    Suspense,
    useContext,
                        ...
                        <div className="space-y-4 max-w-2xl">
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="text-(--text-secondary) text-sm sm:text-base leading-relaxed">
                                    Browse through our full directory of engineering write-ups, deep-dives, and personal notes.
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="text-center py-28 border border-dashed border-(--border) bg-(--bg-secondary)/20 rounded-3xl flex justify-center flex-col items-center space-y-4">
                    ...
                    <div className="space-y-1">
                        ...
                        <p className="text-(--text-secondary) text-xs sm:text-sm max-w-sm mx-auto">
                            Try adjusting your keyword or clearing filters to locate what you need.
                        </p>
                    </div>
                </div>
            ) : (
...
// Subcomponent with stateful image fallback handling
function ArticleCard({ post }: { post: any }) {
    const [imageError, setImageError] = useState(false);

    return (
        <article className="group flex">
            <Link to="/posts/$postId" params={{ postId: post.id.toString() }} className="w-full flex">
                <Card className="w-full bg-(--bg-secondary)/40 hover:bg-(--bg-secondary)/80 border-(--border) hover:border-(--link)/50 transition-all duration-300 hover:shadow-2xl hover:shadow-(--link)/10 flex flex-col overflow-hidden rounded-3xl">
                    ...
                    <CardHeader className="p-6 pb-3 space-y-3 flex-1">
                        <div className="flex flex-wrap gap-1.5">
                            {post.category ? (
                                post.category.split(",").map((cat: string) => (
                                    <span
                                        key={cat}
                                        className="rounded-full bg-(--bg) border border-(--border) px-3 py-1 text-[11px] font-bold text-(--link) capitalize tracking-wide shadow-sm"
                                    >
                                        {cat.trim()}
                                    </span>
                                ))
